...
2. **New → Blueprint** → выберите этот репозиторий, ветку `main`.
3. Подтвердите создание сервиса — Render соберёт Docker-образ и выдаст URL
   вида `https://dragon-academy-api.onrender.com/docs`.

`SECRET_KEY` для JWT сгенерируется автоматически. Нюансы бесплатного тарифа:
сервис засыпает после ~15 минут без трафика (первый запрос после простоя
отвечает ~минуту), а SQLite-база сбрасывается к демо-данным при каждом
рестарте — для учебной песочницы это нормально.
